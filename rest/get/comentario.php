...
//   rest/comentario/?idr={ID_RUTA}  -> devuelve un array con todss los comentarios de la ruta cuyo id es el que se le pasa
...
// =================================================================================
if(is_numeric($ID))
{ // Se debe devolver la información del registro
	$mysql  = 'select * from COMENTARIO where ID=' . mysqli_real_escape_string($link,$ID);
}
else
{ // Se utilizan parámetros
	if(isset($PARAMS['idr']) && is_numeric($PARAMS['idr']))
	{ // se piden todas las fotos de una salida
		$mysql = 'select * from COMENTARIO where ID_RUTA=' . mysqli_real_escape_string($link,$PARAMS['idr']) . ' order by FECHA desc';
	}
  elseif(isset($PARAMS['u']) && is_numeric($PARAMS['u']))
  { // se piden todas las fotos de una salida
    $mysql  = 'select c.*,r.NOMBRE as NOMBRE_RUTA from COMENTARIO c, RUTA r ';
    $mysql .= 'where c.ID_RUTA=r.ID ';
    $mysql .= 'order by c.FECHA desc LIMIT 0,' . mysqli_real_escape_string($link,$PARAMS['u']);
  }
	else
	{
